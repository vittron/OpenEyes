...
<aside class="large-2 column sidebar episodes-and-events">
								...
								<?php	foreach ($episode->events as $event) {
									...
									<li id="eventLi<?php echo $event->id ?>"<?php if ($highlight) { ?> class="selected"<?php }?>>
										...
										<a href="<?php echo $event_path . $event->id ?>" data-id="<?php echo $event->id ?>">
											...
											<span class="event-date" id="eventDate_<?php echo $event->id ?>"> <?php echo $event->NHSDateAsHTML('created_date'); ?></span>
											<?php 
										$this->widget('zii.widgets.jui.CJuiDatePicker',array(
										    ...
										    'options'=>array(
										        ...
										        function(){
										        	...
										        	jQuery.ajax({'dataType':'json','data':{'eventDate': $('#editDate_".$event->id."').val()},'type':'post','success':function(result) {
										        if (result == true) {
					location.href = '".json_encode($this->createUrl('/patient/episode', array('id' => $episode->id)))."';
				} else {
					alert('Sorry! Something went wrong. Please try again later.');
				}
										        },'url':'".json_encode($this->createUrl('/patient/editEpisodeEventDate'))."','cache':false});return false;}
										...
										<span rel="<?php echo $event->id ?>" class="editEventDate"><img src="<?php echo $assetpath . 'img/small_calendar.png' ?>" alt="edit" /></span>
										</a>
	...
	<script type="text/javascript">
		...
		$('.editEventDate').on('click',function(e){
			...
			$('#eventDate_'+id).hide();
			$('#editDate_'+id).show().datepicker('show');
		});
	</script>
</aside>
