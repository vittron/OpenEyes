...
<aside class="large-2 column sidebar episodes-and-events">
								...
								<?php	foreach ($episode->events as $event) {
									...
									<li id="eventLi<?php echo $event->id ?>"<?php if ($highlight) { ?> class="selected"<?php }?>>
										...
										<a href="<?php echo $event_path . $event->id ?>" data-id="<?php echo $event->id ?>">
											...
											<span class="event-date" id="eventDate_<?php echo $event->id ?>" style="padding-left:0"> <?php echo $event->NHSDateAsHTML('created_date'); ?></span>
											<?php 
										$this->widget('zii.widgets.jui.CJuiDatePicker',array(
										    'name'=>'Event_date',
										    'value' => date('d M Y',strtotime($event->created_date)),
										    'id'=>'editDate_'.$event->id,
										    ...
										    'options'=>array(
										        ...
										        'onClose' => "js:
										        function(){
										        	$('#eventDate_".$event->id."').show();
													$('#editDate_".$event->id."').hide();	
													if($(this).val() == '' || $(this).val() == '".date('d M Y',strtotime($event->created_date))."'){
														return false;
													}
										        	jQuery.ajax({'dataType':'json','data':{'eventDate': $('#editDate_".$event->id."').val(), 'event_id':".$event->id.", 'episode_id':".$episode->id.",'YII_CSRF_TOKEN': '".Yii::app()->request->csrfToken."'},'type':'post','success':function(result) {
										        if (result == true) {
					location.href = '".$this->createUrl('/patient/episode', array('id' => $episode->id))."';
				} else {
					alert('Sorry! Something went wrong. Please try again later.');
				}
										        },'url':'".$this->createUrl('/patient/editEpisodeEventDate')."','cache':false});return false;}
										...
										<span rel="<?php echo $event->id ?>" class="editEventDate"><img src="<?php echo $assetpath . 'img/small_calendar.png' ?>" alt="edit" title="Click to Update Event Date"/></span>
										</a>
	...
	<script type="text/javascript">
		...
		$('.editEventDate').on('click',function(e){
			e.preventDefault();
			var id = $(this).attr('rel');
			$('#eventDate_'+id).hide();
			$('#editDate_'+id).css('display','inline').datepicker('show');
		});
	</script>
</aside>
