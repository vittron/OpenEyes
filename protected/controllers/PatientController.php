...
class PatientController extends BaseController
{
	...
	public function accessRules()
	{
		return array(
			...
			array('allow',
				'actions' => array('episode', 'episodes', 'hideepisode', 'showepisode', 'editEpisodeEventDate'),
			...
			array('allow',
				'actions' => array('editPatientInfo','editGpInfo'),  // checked in action
	...
	public function actionView($id)
	{
		...
		// load the info about General Practitioner @nabin
		if(!$contact = $this->patient->contact){
			$contact = new Contact;
		}
		if(!$contact->address)
			$contact->address = new Address;
		if(!$gp = $this->patient->gp){
			$gp = new Gp;
			$gp_contact = new Contact;
			$gp_contact->address = new Address;
			if (Yii::app()->params['default_country']) {
				$gp_contact->address->country_id = Country::model()->find('name=?',array(Yii::app()->params['default_country']))->id;
			} else {
				$gp_contact->address->country_id = 1;
			}
			$gp->contact = $gp_contact;
		}
		if(!$practice = $this->patient->practice){
			$practice = new Practice;
			$practice_contact = new Contact;
			$practice_contact->address = new Address;
			if (Yii::app()->params['default_country']) {
				$practice_contact->address->country_id = Country::model()->find('name=?',array(Yii::app()->params['default_country']))->id;
			} else {
				$practice_contact->address->country_id = 1;
			}
			$practice->contact = $practice_contact;
		}
	...
	public function actionUpdateepisode($id)
	{
		...
		if (!empty($_POST)) {
			if ((@$_POST['eye_id'] && !@$_POST['DiagnosisSelection']['disorder_id'])) {
				...
				// check if start date is changed and set flag @nabin
				if ($_POST['episode_start_date'] != date('d M Y',strtotime($this->episode->start_date))) {
					$episode_update = 1;
 					$this->episode->start_date = date('Y-m-d',strtotime($_POST['episode_start_date']));
				}
				// check if episode is updated and proceed @nabin
				if(isset($episode_update) && $episode_update == 1){
	...
	// Updates patient information, works for two forms, General and Contact info of Patient @nabin
	public function actionEditPatientInfo()
	{
		if (!$patient = Patient::model()->findByPk(@$_POST['patient_id'])) {
			throw new Exception('Unable to find patient: '.@$_POST['patient_id']);
		}	
		...
		if (!empty($_POST['Patient']) || !empty($_POST['Contact'])) {
			...
			if (!$contact->save()) {
				...
				if (!preg_match('/^[0-9]+ [A-Z][a-z]{2} [0-9]{4}$/',$_POST['Patient']['date_of_death']) && strtotime($_POST['Patient']['date_of_death'])) {
					$_POST['Patient']['date_of_death'] = date('j M Y',strtotime($_POST['Patient']['date_of_death']));
				}
				else
					$_POST['Patient']['date_of_death'] = null;

				$patient->attributes = Helper::convertNHS2MySQL($_POST['Patient']); 
				$patient->contact_id = $contact->getPrimaryKey();
				if (!$patient->validate(null,true,array('Contact','Address'))) {
					...
					$address->contact_id = $contact->getPrimaryKey();
					if (!$address->save()) {
	...
	// Updates episode event date information @nabin
	public function actionEditEpisodeEventDate()
	{
		if (!$event = Event::model()->findByPk(@$_POST['event_id'])) {
			throw new Exception('Unable to find event: '.@$_POST['event_id']);
		}	
		$event->created_date = date('Y-m-d', strtotime(@$_POST['eventDate']));
		if (!$event->save()) {
			$form_errors = array();
			foreach ($event->getErrors() as $errors) {
        		foreach ($errors as $error) {
        			$form_errors['Event'][] = $error;
        		}
        	}
        	$result = $form_errors;
		}
	...
	// updates General Practitioner form of patient @nabin
	public function actionEditGpInfo()
	{
		if (!$patient = Patient::model()->findByPk(@$_POST['patient_id'])) {
			throw new Exception('Unable to find patient: '.@$_POST['patient_id']);
		}
		...
		if (!empty($_POST['Contact'])) {
			$gp->contact->attributes = $_POST['Contact'];
            $gp->contact->contact_label_id = 2;
			if (!$gp->contact->save()) {
				// throw new Exception("Unable to save patient address: ".print_r($contact->getErrors(),true));
				foreach ($gp->contact->getErrors() as $errors) {
					foreach ($errors as $error) {
						$form_errors['GP Contact'][] = $error;
					}
				}
			} else {
                $gp->contact_id = $gp->contact->getPrimaryKey();
                if(!$gp->save(false)){
                	// throw new Exception("Unable to save patient address: ".print_r($contact->getErrors(),true));
                	foreach ($gp->getErrors() as $errors) {
                		foreach ($errors as $error) {
                			$form_errors['GP'][] = $error;
                		}
                	}
                }
                else{
                	$patient_update = 1;
					$patient->gp_id = $gp->getPrimaryKey(); 
				}
			}
		}
		if (!empty($_POST['PAddress'])) {
			$practice->contact->attributes = $_POST['PAddress'];
			$practice->contact->contact_label_id = 4;
			if (!$practice->contact->save(false)) {
				// throw new Exception("Unable to save patient address: ".print_r($contact->getErrors(),true));
				foreach ($practice->contact->getErrors() as $errors) {
					foreach ($errors as $error) {
						$form_errors['Practice Contact'][] = $error;
					}
				}
			} else {
				$practice->attributes = $_POST['Practice'];
                $practice->contact_id = $practice->contact->getPrimaryKey();
                if(!$practice->save(false)){
                	// throw new Exception("Unable to save patient address: ".print_r($contact->getErrors(),true));
                	foreach ($practice->getErrors() as $errors) {
                		foreach ($errors as $error) {
                			$form_errors['Practice'][] = $error;
                		}
                	}
                }
                else{
                	$patient_update = 1;
					$patient->practice_id = $practice->getPrimaryKey(); 
				}
			}
		}
		// check if patient model is to be updated
		if(isset($patient_update) && $patient_update == 1){	
			if (!$patient->validate(null,true,array('Contact','Address'))) {
				foreach ($patient->getErrors() as $errors) {
					foreach ($errors as $error) {
						$form_errors['Patient'][] = $error;
				...
				$gp->contact->address->attributes = $_POST['Address'];
				$gp->contact->address->contact_id = $gp->contact->getPrimaryKey();
				$practice->contact->address->attributes = $_POST['PAddress'];
				$practice->contact->address->contact_id = $practice->contact->getPrimaryKey();
				
				if (!$gp->contact->address->save()) {
					throw new Exception("Unable to save GP address: ".print_r($gp->contact->address->getErrors(),true));
				}
				if (!$practice->contact->address->save()) {
					throw new Exception("Unable to save Practice address: ".print_r($practice->contact->address->getErrors(),true));
				}
				$result = true;
			}
		}
