...
class Contact extends BaseActiveRecord
{
  ...
  {
    return array(
      array('first_name, last_name', 'required', 'except' => 'empty'),
      array('nick_name', 'length', 'max' => 80),
      array('title, first_name, last_name, nick_name, primary_phone, secondary_phone, qualifications, contact_label_id', 'safe'),
  ...
  public function relations()
  {
    return array(
      'consultant' => array(self::HAS_ONE, 'Consultant', 'contact_id'),
            'patient' => array(self::HAS_ONE, 'Patient', 'contact_id'),
  ...
  public function getType()
  {
    ...
    /**
     * Get full name for contacts alphabetical order
     * @return CActiveRecord[]
     */
    public static function contactFullNameList()
    {
        $criteria = new CDbCriteria();
        $criteria->select = '`id`, CONCAT(`title`, \' \', `first_name`, \' \', `last_name`) as `full_name`';
//        $criteria->with = 'address';
        $criteria->addCondition('CONCAT(`title`, \' \', `first_name`, \' \', `last_name`) IS NOT NULL');
        $criteria->addCondition('CONCAT(`title`, \' \', `first_name`, \' \', `last_name`) != \'\'');
        $criteria->order = '`title`, `first_name`, `last_name`';
