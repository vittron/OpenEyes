...
class Patient extends BaseActiveRecord
{
	...
	public function rules()
	{
		return array(
			...
			array('no_id', 'hos_num_empty'),
			array('gender', 'length', 'max' => 1),
			array('dob, date_of_death, ethnic_group_id, no_id, no_dp', 'safe'),
			array('dob, hos_num, nhs_num, date_of_death', 'safe', 'on' => 'search'),
		);
	}

	public function hos_num_empty($attribute)
	{
		if (empty($this->no_id) && empty($this->no_dp)) {
			$this->addError($attribute, 'Needs either the DP or ID of the person');
		}
	}
	...
	*/
	...
	{
		$this->nhs_num = $this->no_id . $this->no_dp;
		$lastPatient = self::model()->find(array('order' => 'id DESC'));
		$this->hos_num = (!empty($lastPatient)) ? $lastPatient['id'] + 1 : 1;
	...
	*/
    ...
    public function getSecondary_phone()
    {
        return $this->contact->secondary_phone;
    }
